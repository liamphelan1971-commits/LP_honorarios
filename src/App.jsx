...
function Configuracion({ config, onChange }) {
  ...
  const apiKeyFromEnv = typeof import.meta !== "undefined" && import.meta.env?.VITE_ANTHROPIC_API_KEY;
  ...
  return (
    <div>
      ...
      {/* API Key status */}
      <div style={{ marginBottom:18, padding:"12px 14px", borderRadius:10, background: apiKeyFromEnv ? "#F0FDF4" : "#FEF9C3", border: `1px solid ${apiKeyFromEnv ? "#BBF7D0" : "#FDE68A"}` }}>
        <p style={{ fontSize:12, fontWeight:700, color: apiKeyFromEnv ? "#15803D" : "#92400E", margin:"0 0 4px" }}>
          {apiKeyFromEnv ? "✓ API Key configurada" : "⚠️ API Key no encontrada"}
        </p>
        <p style={{ fontSize:11, color: apiKeyFromEnv ? "#166534" : "#78350F", margin:0 }}>
          {apiKeyFromEnv
            ? "La conexión con la IA está activa. La clave se lee desde la variable de entorno VITE_ANTHROPIC_API_KEY."
            : "Añade VITE_ANTHROPIC_API_KEY en las variables de entorno de Vercel para activar la generación con IA."}
        </p>
      </div>
...
export default function App() {
  ...
  // API key: from env (standalone) or injected by claude.ai (artifact mode)
  ...
  const generarPropuesta = async () => {
    ...
    try {
      ...
      // anthropic-dangerous-direct-browser-access needed for direct browser calls
      ...
      setError(`Error: ${e.message}. Comprueba que VITE_ANTHROPIC_API_KEY está configurada en Vercel.`);
    } finally {
      setLoading(false);
    }
  };
