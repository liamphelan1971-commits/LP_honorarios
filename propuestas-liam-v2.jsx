...
function Spinner({ label = "Generando…" }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 8, color: "#6B7280", fontSize: 13 }}>
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ animation: "spin 0.9s linear infinite", flexShrink: 0 }}>
...
function Toast({ msg, visible }) {
  return (
    <div style={{
      position: "fixed", bottom: 24, left: "50%", transform: `translateX(-50%) translateY(${visible ? 0 : 60}px)`,
      background: "#111827", color: "#fff", padding: "10px 20px", borderRadius: 30,
      fontSize: 13, fontWeight: 600, transition: "transform 0.25s ease", pointerEvents: "none",
    ...
    <div style={{ display: "flex", borderBottom: "1.5px solid #E5E7EB", marginBottom: 24, gap: 0 }}>
      {tabs.map(t => (
        <button key={t.id} onClick={() => onChange(t.id)} style={{
          ...
          background: "none", border: "none", borderBottom: active === t.id ? "2px solid #2563EB" : "2px solid transparent",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {TIPOS.map(t => (
          <button key={t.id} onClick={() => onChange(t.id)} style={{
            ...
            fontWeight: value === t.id ? 700 : 400, fontSize: 12.5,
  ...
  return (
    <div>
      <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#EFF6FF", color: "#1D4ED8", fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, marginBottom: 18 }}>
        <span>{tipoObj?.emoji}</span> {tipoObj?.label}
      </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {[["es", "🇪🇸 Español"], ["en", "🇬🇧 English"], ["both", "🇪🇸+🇬🇧 Bilingüe"]].map(([v, l]) => (
            <button key={v} onClick={() => onIdioma(v)} style={{
              flex: 1, padding: "8px 0", borderRadius: 8, fontSize: 12, fontWeight: idioma === v ? 700 : 400,
              border: idioma === v ? "2px solid #2563EB" : "1.5px solid #E5E7EB",
              background: idioma === v ? "#EFF6FF" : "#fff", color: idioma === v ? "#1D4ED8" : "#374151",
              cursor: "pointer",
            }}>{l}</button>
          ))}
        </div>
      ...
      <Field label="Referencia / nombre del proyecto">
        <input style={inputStyle} value={datos.nombre || ""} onChange={e => onChange("nombre", e.target.value)} placeholder="ej. Vafaey – Calle Las Palmeras 23" />
      </Field>
      <Field label="Cliente">
        <input style={inputStyle} value={datos.cliente || ""} onChange={e => onChange("cliente", e.target.value)} placeholder="Nombre completo o empresa" />
      </Field>
      <Field label="Dirección del inmueble">
        <input style={inputStyle} value={datos.direccion || ""} onChange={e => onChange("direccion", e.target.value)} placeholder="Calle, número, municipio" />
      </Field>

      {["obra_nueva", "reforma"].includes(tipo) && (
        <Field label="Superficie aproximada (m²)">
          <input style={inputStyle} type="number" value={datos.superficie || ""} onChange={e => onChange("superficie", e.target.value)} placeholder="ej. 185" />
        </Field>
      )}
      {["obra_nueva", "reforma", "direccion"].includes(tipo) && (
        <Field label="PEM estimado (€)" hint="Presupuesto de ejecución material">
          <input style={inputStyle} type="number" value={datos.pem || ""} onChange={e => onChange("pem", e.target.value)} placeholder="ej. 280000" />
        </Field>
      )}

      <Field label="Fecha de emisión">
        <input style={inputStyle} type="date" value={datos.fecha || today()} onChange={e => onChange("fecha", e.target.value)} />
      </Field>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          {(FASES_POR_TIPO[tipo] || []).map(f => (
            <label key={f.id} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13, color: "#374151" }}>
              <input type="checkbox" checked={fases[f.id] ?? f.default} onChange={() => onToggleFase(f.id)} style={{ accentColor: "#2563EB", width: 15, height: 15, flexShrink: 0 }} />
              {f.label}
            </label>
          ))}
        </div>
      ...
      <Field label="Honorarios por fase (€ sin IVA)">
        ...
        {totalNeto > 0 && (
          <div style={{ marginTop: 12, background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: 10, padding: "10px 14px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#6B7280", marginBottom: 3 }}><span>IVA 21%</span><span>{eur(totalIVA)}</span></div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, fontWeight: 700, color: "#15803D" }}><span>Total</span><span>{eur(totalNeto + totalIVA)}</span></div>
          </div>
        )}
      </Field>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          {pagos.map((p, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input value={p.hito} onChange={e => {
                const next = [...pagos]; next[i] = { ...p, hito: e.target.value };
                onChange("__pagos", next);
              }} placeholder="Hito de pago" style={{ ...inputStyle, flex: 2, marginBottom: 0 }} />
              <input type="number" value={p.pct} onChange={e => {
                const next = [...pagos]; next[i] = { ...p, pct: e.target.value };
                onChange("__pagos", next);
              ...
              <button onClick={() => { const next = pagos.filter((_, j) => j !== i); onChange("__pagos", next); }}
          ...
          <p style={{ fontSize: 11, color: "#9CA3AF", margin: 0 }}>Suma actual: {pagos.reduce((a, p) => a + (parseFloat(p.pct) || 0), 0)}%</p>
        </div>
      </Field>

      <Field label="Notas para la IA" hint="Instrucciones específicas, cláusulas especiales, contexto del cliente…">
        <textarea value={datos.notas || ""} onChange={e => onChange("notas", e.target.value)} rows={3}
          placeholder="ej. cliente anglófono, añadir cláusula de revisión si el alcance cambia, hacer énfasis en el plazo de prescripción…"
  ...
  return (
    <div>
      ...
      <textarea value={texto} onChange={e => onChange(e.target.value)}
        style={{ width: "100%", minHeight: 520, padding: 16, borderRadius: 10, border: "1.5px solid #E5E7EB", fontSize: 13, lineHeight: 1.75, color: "#111827", background: "#FAFAFA", outline: "none", resize: "vertical", boxSizing: "border-box", fontFamily: "'Georgia', serif" }} />
    </div>
  );
}
...
function Historial({ onLoad, onToast }) {
  const [lista, setLista] = useState(loadHistorial);

  const borrar = (id) => {
    const next = lista.filter(p => p.id !== id);
    setLista(next); saveHistorial(next); onToast("Propuesta eliminada");
  };
...
function Configuracion({ config, onChange }) {
  const field = (label, key, placeholder) => (
    <div style={{ marginBottom: 14 }}>
      ...
      <input style={inputStyle} value={config[key] || ""} onChange={e => onChange(key, e.target.value)} placeholder={placeholder} />
    </div>
  );

  return (
    <div>
      <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 18 }}>Datos que se usan en todas las propuestas.</p>
      ...
      {field("Email", "email", "liam@liamArquitectura.es")}
      {field("Teléfono", "telefono", "+34 600 000 000")}
      {field("NIF / CIF", "nif", "XXXXXXXXX")}
      {field("Dirección profesional", "dir_pro", "Málaga, España")}
      <div style={{ marginBottom: 14 }}>
        <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#374151", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.04em" }}>Cláusula de responsabilidad (pie de propuesta)</label>
        <textarea value={config.clausula || ""} onChange={e => onChange("clausula", e.target.value)} rows={3}
          placeholder="Los presentes honorarios son orientativos y podrán revisarse si el alcance del encargo varía sustancialmente…"
          style={{ ...inputStyle, resize: "vertical" }} />
      </div>
...
function imprimirPDF(texto, nombre) {
  ...
  win.document.write(`
    ...
    <title>${nombre || "Propuesta"}</title>
    <style>
      body { font-family: Georgia, serif; font-size: 13px; line-height: 1.8; color: #111; max-width: 680px; margin: 40px auto; padding: 0 24px; }
      pre { white-space: pre-wrap; word-wrap: break-word; font-family: inherit; }
      @media print { @page { margin: 30mm 20mm; } }
    </style></head><body><pre>${texto.replace(/</g,"&lt;")}</pre>
...
function buildPrompt(tipo, datos, fases, pagos, idioma, config) {
  ...
  const fasesTexto = fasesActivas.map(f => {
    ...
    return `  - ${f.label}: ${h > 0 ? eur(h) + " + IVA" : "(importe pendiente de confirmar)"}`;
  }).join("\n");

  const pagosTexto = pagos.map(p => `  - ${p.pct}% — ${p.hito}`).join("\n") || "  - 40% inicio / 40% hito intermedio / 20% entrega";

  const profInfo = `
Arquitecto: ${config.nombre || "Liam Phelan"}
Estudio: ${config.estudio || "Liam Arquitectura"}
Colegiación: ${config.colegiacion || "COAMÁLAGA nº 1620"}
Email: ${config.email || ""}
Tel: ${config.telefono || ""}
NIF: ${config.nif || ""}
Domicilio profesional: ${config.dir_pro || "Málaga, España"}`.trim();

  const idiomaInstr = idioma === "en"
    ? "Write the entire proposal in English only."
    : idioma === "both"
    ? "Write the proposal in BOTH Spanish AND English. First the full Spanish version, then a horizontal divider (---), then the full English version."
    : "Redacta la propuesta íntegramente en español.";

  return `Eres el asistente del siguiente arquitecto:
${profInfo}
...
RESUMEN ECONÓMICO:
  ...
  IVA (21%):      ${eur(totalIVA)}
  TOTAL:          ${eur(totalBruto)}

CONDICIONES DE PAGO:
${pagosTexto}

NOTAS / INSTRUCCIONES ADICIONALES: ${datos.notas || "ninguna"}

CLÁUSULA PIE: ${config.clausula || "Los presentes honorarios podrán revisarse si el alcance del encargo varía sustancialmente respecto a lo descrito."}

ESTRUCTURA REQUERIDA:
1. Encabezado con datos del arquitecto y fecha
2. Saludo profesional dirigido al cliente (por su nombre si está disponible)
3. Objeto del encargo (2–3 frases claras)
4. Lista de fases con honorarios correspondientes
5. Resumen económico (base / IVA / total)
6. Condiciones de pago
7. Cláusula de pie
8. Firma: nombre, colegiación

ESTILO: profesional, directo, sin florituras, sin bullet points decorativos, lenguaje técnico-arquitectónico.
IDIOMA: ${idiomaInstr}`;
}
...
export default function App() {
  ...
  const generarPropuesta = async () => {
    ...
    try {
      ...
      setError("Error al conectar con la IA. Comprueba tu conexión e inténtalo de nuevo.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const copiar = () => {
    navigator.clipboard.writeText(propuesta).then(() => showToast("✓ Texto copiado"));
  };

  const cargarDesdeHistorial = (p) => {
    setTipo(p.tipo_id || p.tipo);
    setDatos(p.datos || { nombre: p.nombre, fecha: p.fecha });
    setFases(p.fases || {});
    setPagos(p.pagos || DEFAULT_PAGOS);
    setIdioma(p.idioma || "es");
    setPropuesta(p.texto || "");
    setStep(p.texto ? 3 : 2);
    setTab("nueva");
      ...
      <div style={{ maxWidth: 580, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <h1 style={{ fontSize: 19, fontWeight: 700, color: "#111827" }}>
                {config.estudio || "Liam Arquitectura"}
              </h1>
            ...
            <>
              ...
              {step === 1 && (
                <>
                  ...
                  <button onClick={() => setStep(2)} disabled={!tipo} style={{
                    marginTop: 20, width: "100%", padding: "12px 0", borderRadius: 10, border: "none",
                    background: tipo ? "#2563EB" : "#E5E7EB", color: tipo ? "#fff" : "#9CA3AF",
                    fontWeight: 700, fontSize: 14, cursor: tipo ? "pointer" : "default",
                  }}>Continuar →</button>
                </>
              )}
              ...
              {step === 2 && (
                <>
                  <StepDatos tipo={tipo} datos={datos} onChange={setDato} fases={fases} onToggleFase={toggleFase}
                    pagos={pagos} idioma={idioma} onIdioma={setIdioma} />
                  {error && <p style={{ color: "#DC2626", fontSize: 12, marginBottom: 12, padding: "8px 12px", background: "#FEF2F2", borderRadius: 8 }}>{error}</p>}
                  <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
                    <button onClick={() => setStep(1)} style={{ flex: 1, padding: "11px 0", borderRadius: 10, border: "1.5px solid #E5E7EB", background: "#fff", color: "#374151", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>← Atrás</button>
                    <button onClick={generarPropuesta} disabled={loading || !datos.nombre} style={{
                      flex: 2, padding: "11px 0", borderRadius: 10, border: "none",
                      background: loading || !datos.nombre ? "#E5E7EB" : "#2563EB",
                      color: loading || !datos.nombre ? "#9CA3AF" : "#fff",
                      fontWeight: 700, fontSize: 14, cursor: loading || !datos.nombre ? "default" : "pointer",
              ...
              {step === 3 && (
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginTop: 14 }}>
                    <button onClick={reset} style={{ padding: "10px 0", borderRadius: 9, border: "1.5px solid #E5E7EB", background: "#fff", color: "#374151", fontWeight: 600, fontSize: 12, cursor: "pointer" }}>
                      Nueva
                    </button>
                    <button onClick={generarPropuesta} disabled={loading} style={{ padding: "10px 0", borderRadius: 9, border: "1.5px solid #BFDBFE", background: "#EFF6FF", color: "#1D4ED8", fontWeight: 600, fontSize: 12, cursor: "pointer" }}>
                      {loading ? "…" : "Regenerar"}
                    </button>
                    <button onClick={guardarEnHistorial} style={{ padding: "10px 0", borderRadius: 9, border: "1.5px solid #D1FAE5", background: "#ECFDF5", color: "#065F46", fontWeight: 600, fontSize: 12, cursor: "pointer" }}>
                      Guardar
                    </button>
                    <button onClick={copiar} style={{ padding: "10px 0", borderRadius: 9, border: "none", background: "#2563EB", color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                      Copiar
                    </button>
                  </div>
                  <button onClick={() => imprimirPDF(propuesta, datos.nombre)} style={{
